...
def sentiment_analysis(ds=None,**kwargs):
    ...
    for file in files:
        ...
        if outfile.startswith(f'{DATA_PATH}business_world'):
            df['polarity_score'] = df['content'].apply(lambda x: analyze_sentiment(x))

        else:
            df['polarity_score'] = df['summary'].apply(lambda x: analyze_sentiment(x))

        df["sentiment"] = df["polarity_score"].apply(lambda x: label_polarity(x["compound"]))
        df.to_csv(outfile, index=False)
# NER
...
tdag_start >> \
    ...
    >> t2 >> [word_count(), spacy_ner(),sentiment_analysis()] >> t2_end \
    >> [t3 >> [load_data()] >> t3_end] \
